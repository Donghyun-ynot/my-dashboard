...
export default function Dashboard() {
  ...
  function getTasksForDate(day: number) {
    ...
    return tasks.filter(t => t.start_date && t.end_date && t.start_date <= dateStr && dateStr <= t.end_date);
  }
          ...
          <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {["전체","예정","진행중","완료","지연"].map(s => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                      className={`flex-shrink-0 text-xs px-3 py-1.5 rounded-full font-medium border transition ${
                        statusFilter === s ? "bg-indigo-500 text-white border-indigo-500" : "bg-white text-gray-500 border-gray-200"
                      }`}>
            ...
            {activeTab === "캘린더" && (
              <div className="flex gap-4">
                {/* 캘린더 */}
                <div className={`bg-white rounded-xl border p-4 ${selectedCalTask ? "w-1/2" : "w-full"} transition-all`}>
                    ...
                    {Array.from({ length:daysInMonth }).map((_,i) => {
                      ...
                      return (
                        <div key={day} className="min-h-[70px] border border-gray-100 rounded p-0.5">
                          <p className="text-xs text-gray-500 mb-0.5 px-0.5">{day}</p>
                          {dayTasks.map(task => (
                            <div key={task.id}
                              className="rounded mb-0.5 px-1 py-0.5 cursor-pointer hover:opacity-80"
                              style={{
                                backgroundColor: personColorMap[task.person] + "22",
                                borderLeft: `3px solid ${personColorMap[task.person]}`,
                              }}
                              onClick={() => setSelectedCalTask(selectedCalTask?.id === task.id ? null : task)}>
                              <p className="truncate leading-tight"
                                style={{
                                  fontSize:"8px",
                                  color: personColorMap[task.person],
                                }}>
                                {task.person}
                              </p>
                              <p className="truncate leading-tight"
                ...
                {/* 캘린더 상세 패널 */}
                {selectedCalTask && (
                  <div className="w-1/2 bg-white rounded-xl border p-4 space-y-3">
          ...
          {[
            ...
            <button key={tab.label} onClick={() => setActiveTab(tab.label)}
              className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-lg transition ${
                activeTab === tab.label ? "text-indigo-600" : "text-gray-400"
              }`}>
              {tab.icon}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={saveEdit}
                className="flex-2 flex-1 bg-indigo-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-indigo-600 transition flex items-center justify-center gap-1">
